...
|---|---|
| **AVANT** de lancer `watch.py` (s'il y a des entrées `still_pending` à traiter) | Poste un message dans `CRAIG_HOME_CHANNEL` : « ⏳ Je traite N recording(s) Craig en attente : `<craig_id_1>`, `<craig_id_2>`… (Drive cook + Groq, 1-15 min). Je reviens avec le résultat. ». **Mémorise son `message_id`** pour l'édition finale. |
...
- Si `processed` non vide : la notif principale est l'**édition** du message de status (cf. step 2.5) avec le ✅ final, contenant filename + pages wiki updatées. Pas de récap séparé dans `CRAIG_HOME_CHANNEL` — le user lit le status message édité et c'est tout.
...
f. **Lis le JSON de sortie** :
   - `posted` → silence (le recap est sur `CRAIG_HOME_CHANNEL`, séparé du status message).
